...
//!                              │                               │
//!                     ┌────────▼─────────┐ strong    ┌─────────▼────────┐
//!                     │ Arc<InnerWindow> ├───────────► Arc<WindowClass> │
//!                     └────────▲─────────┘           └──────────────────┘
//!    Library (Rust)            │
//!  ────────────────────────────┼─────────────────────────────────────────────
//!    System (Win32)            │
...
//! (window chrome close button, etc.) in which case the native windows side
//! will have shut down already and we must clean up resources on our side.
//!
//! #### Rust Window Dropped First
//!
//! ```text
//!       ┌────────┐       ┌──────────────────┐         ┌───────┐
...
use ::windows::{
    ...
    Win32::{
        ...
        UI::WindowsAndMessaging::{
            ...
            LoadCursorA, PostQuitMessage, RegisterClassExA, SetWindowLongPtrA, ShowWindow,
            UnregisterClassA, CREATESTRUCTA, CS_HREDRAW, CS_VREDRAW, CW_USEDEFAULT, GWLP_USERDATA,
            GWLP_WNDPROC, IDC_ARROW, SW_SHOWNORMAL, WINDOW_EX_STYLE, WM_CLOSE, WM_NCCREATE,
            WM_NCDESTROY, WM_PAINT, WNDCLASSEXA, WS_OVERLAPPEDWINDOW,
        },
    },
};
...
mod inner {
    ...
    lazy_static! {
        static ref WINDOW_REGISTRATIONS: Mutex<HashMap<String, Weak<WindowClass>>> =
            Default::default();
    }
...
{
    /// Construct and display a new window.
    pub fn new(dimension: Dimension2D<i32>, title: &str, on_paint: P) -> Result<Arc<Self>> {
    ...
    pub fn hwnd(&self) -> Option<HWND> {
        let val = self.hwnd.load(Ordering::SeqCst);
        if val == 0 {
            None
        } else {
            Some(HWND(val))
    ...
    fn handle_message(&self, umsg: u32, _wparam: WPARAM, _lparam: LPARAM) {
        ...
        match umsg {
            ...
            WM_CLOSE => {
                // TODO: on_quit or should_quit handler?
    ...
    extern "system" fn wnd_proc_setup(
        ...
        if umsg == WM_NCCREATE {
            ...
            // could be happening.
            unsafe { (*self_).hwnd.store(hwnd.0, Ordering::SeqCst) };
